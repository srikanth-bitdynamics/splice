...
class ExpiredUnclaimedRewardBurnInstructionTrigger(
    ...
    tracer: Tracer,
      ...
      svTaskContext.dsoStore.listExpiredUnclaimedRewardBurnInstruction,
  ...
  override def completeTaskAsDsoDelegate(co: Task, controller: String)(implicit
    ...
    for {
      ...
      cmd = dsoRules.exercise(
        _.exerciseDsoRules_ExpireUnclaimedRewardBurnInstruction(
          co.work.contractId,
          controller,
        )
      )
      _ <- svTaskContext
        ...
        .noDedup
        .yieldUnit()
    } yield TaskSuccess("archived expired unclaimed reward burn instruction")
}
